...
def pytest_collection_finish(session: pytest.Session) -> None:
    ...
    # collect from tests/test_code/ are scenario-shape (no banner; the
    # CLI emits its scenario digest at the wrapper level). Items that
    # are neither (e.g., framework self-tests under tests/framework/
    # when --with-framework is the only scope) also get no contract
    # banner -- they have no per-tool dimension to report on.
