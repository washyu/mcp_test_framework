...
- [x] **UX-02**: `--explain` flag adds full per-tool skip reasons (one line per skipped tool, formatted to stay readable at homelab-mcp scale ~70 tools). The `--explain` flag is plumbed through Typer at the framework CLI layer, not forwarded to pytest.
...
- [x] **UX-05**: `-q` / `--quiet` suppresses the digest and per-tool rows; only the final summary line is emitted. Mirrors v1.1's quiet-mode parity.
