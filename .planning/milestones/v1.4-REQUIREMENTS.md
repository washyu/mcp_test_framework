# Requirements Archive: v1.4 Library Mode Delivery
...
- [x] **CLOSE-02**: Operator running `mcp-contracts run --config PATH` sees identical pass/fail signal to operator running `pytest` against the same `[tool.pytest.ini_options] mcp_config_file = PATH` — CLI/library parity gated by a CI test that drives both routes against the same fixture config and asserts equivalent JUnit XML output. (Plan 30-01: tests/framework/parity/test_cli_vs_pytest_route.py)
- [x] **CLOSE-03**: New operator reading the README sees library-mode usage first ("Add to your `pyproject.toml`, set one line in `[tool.pytest.ini_options]`, run pytest"); CLI usage demotes to an "Appendix: CLI usage" section; `docs/LIBRARY-MODE.md` is the primary reference document for the library API surface. (Plan 30-02)
- [x] **CLOSE-04**: Carry-forward live-UAT items from v1.2 / v1.3 close as part of the library-mode dogfood pass (closed via Plan 30-03 capture protocols + 30-UAT.md UAT-1/2/4 pass + UAT-3 retired to 999.4):
...
| CLOSE-03 | Phase 30 | Complete |
| CLOSE-04 | Phase 30 | Complete (UAT-3 retired to 999.4) |
