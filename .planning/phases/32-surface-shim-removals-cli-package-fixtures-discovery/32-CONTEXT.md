...
Delete every v1.4-introduced `sdet`-flavored surface shim across the operator-visible surfaces — package import (`mcp_test_framework.sdet`), Typer CLI flag (`--sdet`), Typer CLI command (`gen-sdet-classes`), test-discovery fallback (`tests/sdet/`), unprefixed fixtures (`config` / `judge` / `client` / `target_tool`), and legacy console-script entry-point (`mcp-test-framework`). Each removal surfaces an operator-tone migration error pointing at the post-v1.4 name (`mcp_test_framework.test_code`, `--test-code`, `gen-test-classes`, `tests/test_code/`, `mcp_*`-prefixed fixtures, `mcp-contracts`). Lands AFTER Phase 31 so the migration-error paths reference the already-cleaned config surface (no `MCPTF_CONFIG_FILE` / `sdet:` / v1-schema verbiage anywhere).
...
- **D-06 (SHIM-07 mechanism — unprefixed fixtures):** **Stub-raise fixtures**, not clean delete. Keep the six unprefixed fixture names registered in `_plugin.py` (currently L435–461 with DeprecationWarning bodies); replace each body with an immediate `pytest.fail(...)` (or equivalent) carrying an operator-tone message naming the `mcp_`-prefixed equivalent. Rationale: success criterion #4 explicitly requires "fail at fixture-resolution time with the prefixed names surfaced in the error" — pytest's stock "fixture 'config' not found" does fuzzy-match but does not guarantee surfacing the prefixed name as the canonical alternative. Alternative (clean delete): considered, fails SC#4 explicit-pointer requirement. **RESEARCH-FOR-PLAN:** decide `pytest.fail` vs raising a custom exception vs returning a marker-injecting `pytest.skip`. Confirm the six aliases at L435–461 cover the full SC#4 set (`config` / `judge` / `client` / `target_tool` plus any others surfaced during scout — the plugin docstring L12 says "six unprefixed deprecation aliases"). EOL: v1.6 capstone deletes the stub-raises entirely; Phase 35 SHIM-09 regression gate must grandfather the six fixture names for v1.5.
...
- `src/mcp_test_framework/_plugin.py` L12 (docstring naming the six aliases), L379, L392 (`mcp_target_tool` references), L435-461 (six unprefixed alias fixtures with DeprecationWarning bodies — `config`, `judge`, `target_tool`, plus presumably `client`, `tool_config`, and one more per the docstring count of 6). SHIM-07 target (D-06). Verify the full alias set during research.
