...
- **D-02:** **Config source: reuse `./config.test.yaml` — same config Phase 27 D-06 dogfood already uses.** Zero new fixture infrastructure. The parity test becomes one more contract-test consumer of the substrate the framework's CI already exercises. Skips cleanly when the stack is unreachable (planner picks the skip marker — see Claude's discretion). Rejected alternatives: dedicated `parity_config.yaml` (adds a new fixture to keep in sync); hermetic stub MCP server (weakens "real stack" fidelity).
- **D-02a (resolution, 2026-05-17):** Phase 30 research surfaced that `config.test.yaml` ships with `tools: {}` (empty allowlist by Phase 27 D-13/D-16 design — framework CI exercises plugin-load wiring without `uvx homelab-mcp`). Under this substrate the parity test compares two empty outcome dicts (vacuous PASS). **Resolution: keep D-02 verbatim; parity test carries a defensive `assert outcomes_a, "<error explaining tools: must be populated for non-vacuous parity>"`** at the top of the comparison block, AND `docs/LIBRARY-MODE.md` + the parity-test docstring document that running the parity gate locally requires populating `tools:` in `config.test.yaml` (or pointing at another config). Honors D-02's "reuse config.test.yaml" letter; SC1 dogfood verification is satisfied independently of the parity payload (Phase 27 D-06 loop is what SC1 re-confirms). Rejected: Option B (dedicated `parity.test.yaml` fixture) and Option C (use `config.yaml`) — both reverse D-02 for less benefit than the defensive-assert path.
