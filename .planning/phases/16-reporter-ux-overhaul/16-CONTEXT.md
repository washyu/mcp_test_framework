...
- **D-02:** [informational] **Crash semantics.** If pytest exits without writing the JUnit XML (Phase 14 D-16), the operator already saw the digest before the crash — `_dispatch_default_mode_or_error` then surfaces the operator-tone error pointing at `--raw`. No regression; the digest is more useful pre-crash than post-crash anyway. (No code change in Phase 16 — inherited Phase 14 behavior; marked informational so the decision-coverage gate doesn't flag it.)
