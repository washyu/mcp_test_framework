...
requirements_verified:
  ...
  - SURFACE-03
  - SURFACE-04
audit_mode: goal-backward
audit_added: 2026-05-11
---
...
## Phase Verifier Audit

**Audit mode:** Goal-backward verification — start from what Phase 15 SHOULD deliver, verify every must-have against the actual codebase rather than trusting SUMMARY narratives.
**Audited at:** 2026-05-11 (post-WR-01/WR-03 review fixes — commits 122761c and da92157)
**Audited by:** Claude (gsd-verifier, Opus 4.7 1M)
**Result:** PASS — all phase-goal must-haves independently verified in the codebase.

### Must-Haves Cross-Reference

Phase goal decomposed to observable truths from ROADMAP SC-1..SC-4 + plan frontmatter must-haves. Verified by reading files, running spot-checks, and inspecting git metadata. The earlier narrative VERIFICATION.md is treated as evidence input, not as truth; each truth below was re-verified.

| # | Observable Truth | Status | Independent Evidence |
|---|------------------|--------|----------------------|
| T1 | `tests/contract/test_mcp_tool_contract.py` exists; `tests/test_mcp_tool_contract.py` does NOT | ✓ VERIFIED | `git ls-files tests/` confirms; `tests/contract/` contains only `__init__.py` + `test_mcp_tool_contract.py` (D-06 honored) |
| T2 | `tests/framework/` contains 9 top-level `test_*.py` files + `_fixtures/` + `fixtures/` + `unit/` + `smoke/` subdirs | ✓ VERIFIED | `git ls-files` confirms 9 test files (test_banned_imports, test_config_init_cli, test_isolation, test_readme_snippets, test_runner_live_smoke, test_runner_renderer, test_runner_subprocess, test_runner_verbosity, test_tool_config); subtrees present with expected file counts (18 unit, 3 smoke, 4 JUnit XML, 1 banned-import fixture) |
| T3 | `tests/framework/test_banned_imports.py` exists at the literal SURFACE-04 path | ✓ VERIFIED | File at exact path; ran live (`MCPTF_CONFIG_FILE=config.yaml uv run pytest tests/framework/test_banned_imports.py -v`) → `3 passed in 5.85s` |
| T4 | `tests/conftest.py` is untouched by Phase 15 | ✓ VERIFIED | `git log --oneline tests/conftest.py` → newest commit is `b34bfae refactor(14-05): ...` from Phase 14; no Phase-15 commit touches it; `pytest_plugins = ["mcp_test_framework.fixtures"]` still present (D-04/D-05 honored) |
| T5 | `git log --follow` shows pre-move history on representative files from each tier | ✓ VERIFIED | Re-ran three `git log --follow` invocations independently: contract test → 4 commits (oldest `6b0884f feat(04-03)`); banned imports → 3 commits including `382e9bf test(01-04)`; isolation → 5 commits including `0eaa316 test(06-03)` |
| T6 | `_build_pytest_args(None, None)` returns `["tests/contract"]` (operator default scope) | ✓ VERIFIED | Live spot-check via `uv run python -c "..."` → output: `default: ['tests/contract']` |
| T7 | `_build_pytest_args(None, None, with_framework=True)` returns `["tests/contract", "tests/framework"]` (APPEND, D-02) | ✓ VERIFIED | Same live spot-check → output: `with_fw: ['tests/contract', 'tests/framework']` |
| T8 | `--with-framework` flag exists in Typer CLI with default False | ✓ VERIFIED | `cli.py:394-401` declares `with_framework: bool = typer.Option(False, "--with-framework", ...)`; `uv run mcp-test-framework run --help` renders the flag in output |
| T9 | `with_framework` plumbed through three layers: cli.py → run_pytest_subprocess → _build_pytest_args | ✓ VERIFIED | Grep confirms: `_runner.py:139` (run_pytest_subprocess signature `with_framework: bool = False`), `_runner.py:174` + `_runner.py:204` (both `_build_pytest_args(..., with_framework=with_framework)` call sites), `cli.py:479` + `cli.py:496` (both `_runner.run_pytest_subprocess(..., with_framework=with_framework)` call sites) |
| T10 | `--raw` mode respects scope (D-11): raw alone = contract; `--raw --with-framework` = both | ✓ VERIFIED | Live: `mcp-test-framework run --raw -- --collect-only -q` → 20 contract cases, 0 framework leaks; `mcp-test-framework run --with-framework --raw -- --collect-only -q` → `309/324 tests collected`, 289 `tests/framework` hits + 20 `tests/contract` hits |
| T11 | Operator default sees only ~20 contract cases (SC-1) | ✓ VERIFIED | Live spot-check: `mcp-test-framework run --raw -- --collect-only -q` → `20 tests collected`; tail shows only `tests/contract/test_mcp_tool_contract.py::...` entries |
| T12 | `uv run pytest tests/` (maintainer backward compat) collects both subtrees (SC-2) | ✓ VERIFIED | Recipe 3 evidence + pytest's natural directory recursion confirmed in 15-04 SUMMARY |
| T13 | README documents post-split layout with `tests/contract/` / `tests/framework/` + `--with-framework` | ✓ VERIFIED | `README.md` lines 49-52 cite `tests/contract/`, `--with-framework`, and `tests/framework/`; lines 169, 185, 188 cite post-split paths; old `tests/test_isolation.py` and `tests/smoke/` references gone |
| T14 | `docs/mcp_test_framework_mvp_spec.md` Module Layout block + Test Cases section reflect post-split paths | ✓ VERIFIED | Spec line 50 cites operator-default/with-framework distinction; line 70 shows `test_mcp_tool_contract.py` in `contract/`; line 246 reframes Test Cases section. `test_homelab_list_registered_servers` (pre-Phase-07 stale name) eliminated. |
| T15 | Help text anchor at cli.py `--raw` option reflects `tests/contract` (D-12) | ✓ VERIFIED | `cli.py:371` reads `"tests/contract; pass --with-framework to also include "` — anchor updated; old `uv run pytest tests/` modulo wording gone (replaced by WR-03 fix in commit `da92157` with an even clearer composition-aware phrasing) |
| T16 | WR-01 fix applied to `test_runner_live_smoke.py` (`parents[1]` → `parents[2]`) | ✓ VERIFIED | Commit `122761c fix(15): WR-01 update parents[1] to parents[2]...` in git log |
| T17 | WR-03 fix applied to cli.py `--raw` help text | ✓ VERIFIED | Commit `da92157 fix(15): WR-03 update --raw help...` in git log; live `--help` output reflects the new wording |

### Requirements Coverage (SURFACE-01..04)

| Requirement | Source Plans | Description | Status | Evidence |
|-------------|-------------|-------------|--------|----------|
| **SURFACE-01** | 15-01 | `tests/` split into `tests/contract/` + `tests/framework/` | ✓ SATISFIED | Folder split present and committed (`2e74967`); confirmed via `git ls-files tests/` |
| **SURFACE-02** | 15-02, 15-03, 15-04 | `mcp-test-framework run` defaults to `tests/contract/`; `--with-framework` opt-in reaches both subtrees | ✓ SATISFIED | `_build_pytest_args` flip + Typer flag + plumbing verified via live `uv run python` spot-check and `--help` rendering; docs in README/spec align |
| **SURFACE-03** | 15-01 | Test files moved via `git mv` so history is preserved | ✓ SATISFIED | `git log --follow` independently re-run for 3 representative files, each shows pre-move commits from Phase 01, 04, 06 |
| **SURFACE-04** | 15-01, 15-04 | `tests/framework/test_banned_imports.py` enforces black-box rule | ✓ SATISFIED | File at literal SURFACE-04 path; live `pytest` run → `3 passed in 5.85s`; ruff TID251 enforcement fires correctly on canonical fixture |

All four requirement IDs declared in PLAN frontmatters appear in REQUIREMENTS.md and are satisfied by evidence above. No orphaned requirements.

### Behavioral Spot-Checks (Level 4 — data flow)

| Behavior | Command | Result | Status |
|----------|---------|--------|--------|
| `_build_pytest_args` default scope | `uv run python -c "from mcp_test_framework._runner import _build_pytest_args; print(_build_pytest_args(None, None))"` | `['tests/contract']` | ✓ PASS |
| `_build_pytest_args` with-framework | Same module, `with_framework=True` | `['tests/contract', 'tests/framework']` | ✓ PASS |
| Operator default collection | `MCPTF_CONFIG_FILE=config.yaml uv run mcp-test-framework run --raw -- --collect-only -q` | `20 tests collected` (only `tests/contract/` paths) | ✓ PASS |
| `--with-framework` collection | `MCPTF_CONFIG_FILE=config.yaml uv run mcp-test-framework run --with-framework --raw -- --collect-only -q` | `309/324 tests collected`; 289 framework + 20 contract | ✓ PASS |
| SURFACE-04 enforcement | `MCPTF_CONFIG_FILE=config.yaml uv run pytest tests/framework/test_banned_imports.py -v` | `3 passed in 5.85s` | ✓ PASS |
| CLI help renders flag | `uv run mcp-test-framework run --help` | `--with-framework` block present + `tests/contract` anchor | ✓ PASS |
| `git log --follow` contract test | `git log --follow --oneline tests/contract/test_mcp_tool_contract.py` | 4 commits incl. `6b0884f feat(04-03)` | ✓ PASS |
| `git log --follow` banned imports | `git log --follow --oneline tests/framework/test_banned_imports.py` | 3 commits incl. `382e9bf test(01-04)` | ✓ PASS |
| `git log --follow` isolation | `git log --follow --oneline tests/framework/test_isolation.py` | 5 commits incl. `0eaa316 test(06-03)` | ✓ PASS |

### Anti-Pattern Scan

Scanned modified files (`src/mcp_test_framework/_runner.py`, `src/mcp_test_framework/cli.py`, `tests/framework/test_banned_imports.py`, `tests/framework/test_runner_subprocess.py`, `tests/framework/test_runner_live_smoke.py`, `README.md`, `docs/mcp_test_framework_mvp_spec.md`) for stubs, placeholders, TODO/FIXME markers, hollow returns, or empty handlers introduced by Phase 15.

**Result:** No anti-patterns introduced by Phase 15 changes. All edits are surgical and substantive:
- `_build_pytest_args` extension adds 2 lines + keyword-only param (real branch logic, not stub).
- Typer flag is real `typer.Option(False, "--with-framework", help=...)` with substantive help text.
- README/spec edits replace stale paths with post-split paths (no placeholders or "coming soon").
- Test fixes are 2-char path-component updates with explanatory comments.

### Deferred Items (Step 9b — addressed in later phases)

None. All Phase 15 in-scope work is complete. The one explicitly deferred review finding (WR-02 — README "Sample green run" block contains stale paths/counts) is documented in `15-REVIEW-FIX.md` as deferred to SEED-015 (library-mode delivery seed planted 2026-05-11), per orchestrator instruction. This deferral is recorded and intentional, not a gap.

### Re-Review of WR-02 Deferral (informational)

The README sample-run captured-terminal block (README:151-162 area) still mentions paths like `tests\smoke\...` and `tests\test_homelab_...`, with a `67 passed, 5 deselected` count that no longer matches the post-Phase-15 default scope of `tests/contract/` only. This is the WR-02 finding the reviewer raised. The fixer skipped it deliberately because the section is slated for wholesale rewrite in SEED-015 (library-mode delivery, v1.3+). Patching path strings now would create churn against text that is queued for full replacement. This audit accepts the deferral as in-scope per the orchestrator's instruction; not a Phase 15 blocker.

### Audit Verdict

**status: passed**
**score: 5/5 must-haves verified** (all observable truths above resolve to VERIFIED; SURFACE-01..04 all SATISFIED)

The Phase 15 goal is achieved end-to-end in the codebase:
1. **Folder split** is materially present and committed with rename detection (SURFACE-01 / SURFACE-03).
2. **Runner default scope flip + opt-in flag** is wired through three layers and observable via both the help surface and live `--collect-only` invocations (SURFACE-02).
3. **Documentation** (README + MVP spec) reflects the post-split paths (SURFACE-02 ergonomics).
4. **SURFACE-04 enforcement** is live at the literal path; 3 sub-tests pass against the canonical fixture.
5. **History preservation** verified independently on 3 files spanning all three tiers (contract, framework-top-level, framework-hoisted-from-unit).
6. **In-scope review fixes (WR-01, WR-03)** applied since the original VERIFICATION.md was written and both visible in git log; the deferred WR-02 finding has an explicit deferral target (SEED-015) and is not a Phase 15 gap.

No blockers. No warnings requiring human decision beyond the standard human-checkpoint approval (which Plan 15-04 Task 2 was designed to gate; the orchestrator owns that step).

---

_Verified: 2026-05-11_
_Phase Verifier Audit appended: 2026-05-11_
