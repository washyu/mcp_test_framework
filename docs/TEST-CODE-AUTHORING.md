...
is unreachable. The test-code author persona is distinct from the operator's
contract pass: operators run the schema/judge/output sweep against every
discovered tool; test-code authors write targeted lifecycle scenarios that
exercise stateful tool sequences end-to-end.
...
- Tests are async-only under `pytest-asyncio` strict mode. Every test-code
  test carries an explicit `@pytest.mark.asyncio(loop_scope="session")` marker —
...
`<test_code.generated_root>/<server_slug>/` tree — where `test_code.generated_root`
is the required path you set in your `config.yaml`. The recommended convention
is `tests/test_code/_generated/` (colocated with your `tests/test_code/`
scenarios; the underscore prefix signals "tool-managed, don't hand-edit").
Every generated file carries a `do not hand-edit` header at the top; if you
need to extend a generated class with a helper, subclass it in your own
scenario file rather than editing the generated source.

The change-detection signal is your type checker. After a regen, run
`mypy` or `pyright` over `tests/test_code/`. Drift between your scenario code
and the regenerated classes surfaces as a type error at the call site that
needs updating — `CreateProxmoxVmParams(...)` rejects a removed field, or a
...
(`from tests.test_code._generated.<server_slug> import <ClassName>` if you
adopt the recommended `test_code.generated_root: tests/test_code/_generated`
convention — see "Importing generated Params and Response classes" below for
the layout mechanics) do not change when the schema underneath does; only the
internal field set of each class changes. Pin your imports against the import
surface, and let the type checker tell you which call sites need adjustment.

## Configuring the generated-classes path

The framework reads where to write generated test-code classes from
`test_code.generated_root` in your `config.yaml`. This field is required; the
...
with your `tests/test_code/` scenarios under an underscore-prefixed
...
recommended `tests.test_code._generated.<slug>` path, drop an empty
`__init__.py` into both `tests/test_code/_generated/` and your operator's own
`tests/__init__.py` / `tests/test_code/__init__.py` so the path resolves as a
Python package.

## Importing generated Params and Response classes

The test-code surface is two namespaces. The first is
`mcp_test_framework.test_code`, which exposes the framework's runtime symbols
(`mcp_session`, `tool`, `ToolCallError`). The second is the per-server
generated module under your operator-configured `test_code.generated_root` — by
convention this is `tests/test_code/_generated/<server_slug>/`, importable as
`tests.test_code._generated.<server_slug>` once you make `tests/test_code/_generated/`
a package in your own test tree (drop an empty `__init__.py` there). The
framework no longer writes generated code inside its own `src/` tree; the
operator controls the location.

```python
from mcp_test_framework.test_code import mcp_session, tool, ToolCallError
...
uniform attributes — the test-code-side access pattern (`response.data["vmid"]`)
...
```python
import pytest
from mcp_test_framework.test_code import mcp_session, tool
from tests.test_code._generated.homelab_mcp import CreateProxmoxVmParams
...
**You generally won't hit this bug.** The framework's `tool().call()`
serializer uses `model_dump(mode="json", exclude_unset=True)`, which means
optional Pydantic fields that you never set on the params constructor stay
off the wire entirely. The contradiction surfaces only when a test-code
author _explicitly_ chooses to put `null` on the wire — either by passing
`field=None` to a constructor, or (for tools whose declared `extra="forbid"`
blocks the action payload shape) by reaching for the `extra="allow"` escape
hatch below.
...
from tests.test_code._generated.homelab_mcp import ManageProxmoxVmParams
...
2. **The framework does not paper over upstream bugs.** The serializer
   chose `model_dump(mode="json", exclude_unset=True)` deliberately over
   `exclude_none=True`. `exclude_unset=True` discriminates on **user
   intent** — did the test-code author set this attribute on the params
   constructor? — not on **value**. A test-code author who explicitly writes
   `field=None` still puts `null` on the wire and still triggers the upstream
   contradiction when one exists. `exclude_none=True` would silently drop
   _every_ null field regardless of whether the test-code author wanted it on
   the wire, masking the upstream contradiction and also preventing
   test-code authors from testing the server's null-handling edge cases when
   they want to. This is the framework-primitives principle: framework
   primitives; test-code author owns safety. The framework wraps tool calls;
   the test-code author decides which payloads to send. See the in-repo
   memory file `project_framework_primitives_sdet_safety_principle.md` for
   the full statement of the principle.

3. **The escape hatch lives in the scenario file.** A `_CpuBump*` subclass
   in `tests/test_code/test_proxmox_vm_lifecycle.py` is local to the workaround
   it enables. The framework's role is to surface upstream contract
   violations as test failures; the test-code author's role is to decide
   whether to work around an upstream bug, file it, or fail loudly. Both are
   valid choices, and both stay visible at the scenario boundary.
...
```python
# tests/test_code/test_provision.py
import pytest
import pytest_asyncio
...
- The framework itself ships no test-code scenarios, so the framework's own
...
against the current environment is the test-code author's call, not the
framework's: the author defines a `_probe()` callable, wires it into
`@pytest.mark.skipif`, and the scenario SKIPs when the probe returns
`False`. This is the framework-primitives principle — framework primitives;
test-code author owns safety — recorded in the in-repo memory file
...
to keep growing to match every test-code author's infrastructure; keeping
`_probe()` author-defined keeps that pressure where it belongs.
...
`.code` / `.message` via the em-dash detail pattern
...
already in place: `tests/test_code/conftest.py` ships a `pytest_exception_interact`
hook that writes the four attributes into the test's
`user_properties` on the JUnit node. Test-code authors do not modify this
file — the hook is framework infrastructure that every authored scenario
inherits.
...
CI-runnable scenarios that exercise the full test-code wiring without
operator infrastructure. Until that fixture ships, the canonical pattern
for keeping infrastructure-bound scenarios honest in CI is the conditional
skip recipe in the section above.

## Further reading

- `docs/EXTENDING.md` — operator-side knobs (rubrics, judge backend, env passthrough).
- `README.md` — operator quickstart and the `## test-code scenarios` sample.
- Generated module: `<test_code.generated_root>/<server_slug>/` (e.g. `tests/test_code/_generated/<server_slug>/` under the recommended convention).
